...
public class Board extends ArrayList<Triad> {
    ...
    public boolean fitsMode() {
        ...
        for (int[] mode : s.modes)
            for (int i = 0; i < superSet.size(); i++) {
                ...
                if (false && !s.allowedTrans.contains(propRoot)) {
                    continue;
                }
                boolean isC = true;
                for (int m : superSet) {
    ...
    public int getMinSyntacticDistance() {
    ...
    public double getAveSyntacticDistance() {
        double average = 0;
        for (int i = 1; i < size(); i++)
    ...
    // @Precondition size() > 0
    public int getDistanceTo(Triad prospectiveFinalTriad) {
